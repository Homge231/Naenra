...
async function seedCoreMissions() {
  console.log('🌱 Starting Core Missions Seeder...')

  // 1. Fetch all cores from Supabase DB
  ...
  for (const core of cores) {
    ...
    // Base Cores are unlocked by default, only Upgrade Cores (Tier 2/3) have unlock missions
    if (isBaseCore) continue

    const nameKey = core.name.toLowerCase().replace(/['']/g, '').replace(/[^a-z0-9]/g, ' ').trim()
    
    // Find matching mission seed
    let seed = MISSION_SEEDS[nameKey]
    if (!seed) {
      // Direct lookup fallback
      const directKey = core.name.toLowerCase()
      seed = MISSION_SEEDS[directKey]
    }

    if (!seed) {
      // Default generic seed for any unmatched upgrade core
      seed = {
    ...
    // Upsert mission into core_missions table
